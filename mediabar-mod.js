...
(function () {
  ...
  /* yearTo is always computed at runtime — never stored */
  var cfg = { minRating:0, count:20, mediaType:'All', interval:8, yearFrom:MIN_YEAR };
  function loadCfg(){
  ...
  style.textContent = `
    ...
    /* ── Year: "from" label + "– This Year" badge ─────────────────────────── */
    .mbrf-year-row {
  ...
  pill.innerHTML = `
    ...
    <div>
      <div class="mbrf-lbl" style="margin-bottom:6px">Release Year <b id="mbrf-yfv">${cfg.yearFrom}</b></div>
      <div class="mbrf-year-row">
        <input id="mbrf-yfs" class="mbrf-slider" type="range" min="${MIN_YEAR}" max="9999" step="1" value="${cfg.yearFrom}" style="flex:1">
        <span class="mbrf-year-badge" id="mbrf-this-year">– This Year</span>
      </div>
    </div>
    <div>
  ...
  var yfs = document.getElementById('mbrf-yfs');

  /* Set slider max to current year right now; re-checked on tryInit too */
  yfs.max = new Date().getFullYear();
  gradSet(rs); gradSet(cs); gradSet(is_); gradSet(yfs);
  renderStars(cfg.minRating);

  rs.addEventListener('input',function(){
    document.getElementById('mbrf-rv').textContent=this.value==0?'–':this.value;
    gradSet(this); renderStars(parseFloat(this.value)); resetPillAutoClose();
  });
  ...
  yfs.addEventListener('input',function(){
    document.getElementById('mbrf-yfv').textContent=this.value;
    gradSet(this); resetPillAutoClose();
  });
  ...
  document.getElementById('mbrf-save').addEventListener('click',function(){
    ...
    cfg.yearFrom  = parseInt(yfs.value);
  ...
  gear.addEventListener('click',function(e){
    ...
    if(pillOpen){
      ...
      pill.style.left=Math.max(10,r.right-260)+'px';
  ...
  function fetchRandom(cb){
    ...
    /* yearTo is always the current year at fetch time — never a stale saved value */
  ...
  function tryInit(){
    ...
    /* Year: update slider max to current year, restore saved From value */
    yfs.max = new Date().getFullYear();
    yfs.value = cfg.yearFrom; gradSet(yfs);
    document.getElementById('mbrf-yfv').textContent = cfg.yearFrom;
